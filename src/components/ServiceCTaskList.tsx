...
export const ServiceCTaskList: React.FC<ServiceCTaskListProps> = ({
  serviceSelection,
  mode,
  currentProgress = [],
  onProgressChange,
  ...
  // Only show if Service C is selected
  if (serviceSelection !== 'Service C') {
    return null;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
        <p className="text-sm text-gray-700">
          ALL <span className="font-bold text-black">Status</span>, <span className="font-bold text-black">Description</span> and <span className="font-bold text-black">Done By</span> fields are <span className="font-bold text-red-600">MANDATORY</span>, and need to be filled out for the Service C Task List
        </p>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full table-fixed">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {SERVICE_C_TASKS.map((task, index) => (
                <ServiceTaskRow
                  key={task}
                  task={task}
                  index={index}
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
        <div className="flex items-start gap-3">
          <Settings className="h-5 w-5 text-emerald-600 mt-0.5 flex-shrink-0" />
          <div>
            <h4 className="text-sm font-medium text-emerald-900">Service C Completion</h4>
            <p className="text-sm text-emerald-700 mt-1">
              Complete all {SERVICE_C_TASKS.length} tasks and mark their status. Add descriptions for any issues found
