...
export const useJobCards = () => {
  ...
  const formatJobCardData = (card: any): JobCard => ({
    ...
    trailer_progress: (card.trailer_progress || []).map((trailer: any) => ({
      ...
      // Handle migration from old 'remarks' field to new structure
      electrical_tasks: (trailer.electrical_tasks || []).map((task: any) => ({
        ...task,
        // Migrate old 'remarks' field to 'description' if needed
        description: task.description || task.remarks || '',
        status: task.status,
        done_by: task.done_by || '',
        hours: task.hours || 0,
      })),
      tires_wheels_tasks: (trailer.tires_wheels_tasks || []).map((task: any) => ({
        ...task,
        description: task.description || task.remarks || '',
        status: task.status,
        done_by: task.done_by || '',
      ...
      brake_system_tasks: (trailer.brake_system_tasks || []).map((task: any) => ({
        ...task,
        description: task.description || task.remarks || '',
        status: task.status,
        done_by: task.done_by || '',
      ...
      suspension_tasks: (trailer.suspension_tasks || []).map((task: any) => ({
        ...task,
        description: task.description || task.remarks || '',
        status: task.status,
        done_by: task.done_by || '',
      ...
      body_chassis_tasks: (trailer.body_chassis_tasks || []).map((task: any) => ({
        ...task,
        description: task.description || task.remarks || '',
        status: task.status,
        done_by: task.done_by || '',
      ...
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          trailer_progress: (newJobCard.trailer_progress || []).map(trailer => ({
            ...
            // Ensure new structure is saved
            electrical_tasks: (trailer.electrical_tasks || []).map(task => ({
              name: task.name,
              task: task.task,
              status: task.status,
            ...
            tires_wheels_tasks: (trailer.tires_wheels_tasks || []).map(task => ({
              name: task.name,
              task: task.task,
              status: task.status,
              description: task.description || '',
            ...
            brake_system_tasks: (trailer.brake_system_tasks || []).map(task => ({
              name: task.name,
              task: task.task,
              status: task.status,
              description: task.description || '',
            ...
            suspension_tasks: (trailer.suspension_tasks || []).map(task => ({
              name: task.name,
              task: task.task,
              status: task.status,
              description: task.description || '',
            ...
            body_chassis_tasks: (trailer.body_chassis_tasks || []).map(task => ({
              name: task.name,
              task: task.task,
              status: task.status,
              description: task.description || '',
